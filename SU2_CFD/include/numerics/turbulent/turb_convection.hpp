...
private:
   ...
   */\
  void FinishResidualCalc(const CConfig* config) override {
      /*std::cout << "FinishResidualCalc: ScalarVar_i[0] = " << ScalarVar_i[0]
          << ", ScalarVar_i[1] = " << ScalarVar_i[1]
         << std::endl;*/
    ...
    Flux[1] = a0*V_i[idx.Density()]*ScalarVar_i[1] + a1*V_j[idx.Density()]*ScalarVar_j[1];
        //std::cout << "Residual Flux: [" << Flux[0] << ", " << Flux[1] << "]\n";
...
private:
  ...
  void FinishResidualCalc(const CConfig* config) override {
    ...
    //std::cout << "Residual Flux: [" << Flux[0] << ", " << Flux[1] << ", " << Flux[2] << "]\n";
