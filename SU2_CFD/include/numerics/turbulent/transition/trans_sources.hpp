...
/*!
 * \class CSourcePieceWise_TransSLM
...
class CSourcePieceWise_TransLMROUGH final : public CNumerics {
 private:
  ...
  su2double duds_Here = 0.0;
  //Addition for LMROUGH
  su2double F_Ar_Here = 0.0;

  su2double IntermittencySep = 1.0;
  su2double IntermittencyEff = 1.0;

  //Modification for LMROUGH to add the third variable in residual and jacobian arrays, for Jacobian buffer from 4 to 9 
  su2double Residual[3]; //holds computation for source terms in intermittency eq (Residual[0]) and re_t eq (Residual[1]), A_r eq (Residual[2])
 ...
 public:
  ...
  CSourcePieceWise_TransLMROUGH(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config)
    ...
    /*--- "Allocate" the Jacobian using the static buffer. ---*/
    //Modification for LMROUGH, now we have 3 variables and we need to allocate the 9 elements into the Jacobian_Buffer
    Jacobian_i[0] = Jacobian_Buffer;
  ...
  ResidualType<> ComputeResidual(const CConfig* config) override {
    ...
    //Expanding the Residual and Jacobian for the third variable 
    Residual[0] = 0.0;
    Residual[1] = 0.0;
    Residual[2] = 0.0;
    Jacobian_i[0][0] = 0.0;
    ...
    if (dist_i > 1e-10) {
      ...
      /*--- Roughness influence function for LMROUGH modle ---*/
      su2double F_Ar1 = 0.0;
      ...
      // ANDREA: Perchè c'è questo if??
      // Ho limitato Re_theta per evitare le oscillazioni 
      // Perchè? Ar ti usciva negativo? Perchè mi sembra che FAr sia sempre positivo no? Quindi qui stai sempre togliendo qualcosa di positivo.
      // Allora fai qualcosa così piuttosto. In questo modo stai facendo quello che fanno loro no? Se retheta è lontano dal valore minimo allora stai moltiplicando
      // per un b circa 1, altrimenti va a 0 per Retheta che si avvicina al valore minimo. Così è una funzione continua, tu invece la fai discontinua
      // e non è molto buona come soluzione.
      // Ok, va bene. In ogni caso prima non modificava l'andamento dello skin friction, o il punto di transizione, ma aveva solo eliminato le oscillazioni.
      // Chiaro, prova per favore a vedere se così cambia qualcosa o no. Guarda anche il commento in CTransLMSolver.cpp sui lowerlimit di retheta
      // da dove hanno preso quel valore minimo? Non ricordo se le relazioni di Retheta lo avevano, ora controllo.
      //Ok controllo, poi ricompilo e lancio la simulazione.
      PRethetat = c_theta *( Density_i / time_scale) * ((Corr_Ret - TransVar_i[1]) * (1.0 - f_theta) - ((TransVar_i[1]-30.0)/TransVar_i[1])*F_Ar1);
      //if (TransVar_i[1] > 50) {
      // PRethetat = c_theta *( Density_i / time_scale) * ((Corr_Ret - TransVar_i[1]) * (1.0 - f_theta) - F_Ar1);
      //} else {
      // PRethetat = c_theta * (Density_i / time_scale) * ((Corr_Ret - TransVar_i[1]) * (1.0 - f_theta));
      //} 
      ...
      Jacobian_i[0][0] = (F_length * c_a1 * StrainMag_i * sqrt(F_onset) *
                         ...
                         Volume;
      Jacobian_i[0][1] = 0.0;
      Jacobian_i[0][2] = 0.0; // no dependecy of Residual[0] from A_r
      Jacobian_i[1][0] = 0.0;
      Jacobian_i[1][1] = -c_theta / time_scale * (1.0 - f_theta) * Volume;
      if (options.CrossFlow && ReThetat_SCF - TransVar_i[1] < 0) {
        Jacobian_i[1][1] += (c_theta / time_scale) * c_CF * f_theta_2 * Volume;
      ...
      Jacobian_i[2][0] = 0.0;  // No dependency of Residual[2] on TransVar_i[0] (Gamma).
      Jacobian_i[2][1] = 0.0;  // No dependency of Residual[2] on TransVar_i[1] (ReThetaT).
      Jacobian_i[2][2] = 0.0;  // No implicit dependency for A_r (Residual[2]).
  ...
  inline su2double GetLambda_theta() override {return lambda_theta_Here;} 
  inline su2double Getduds() override {return duds_Here;} 
  inline su2double GetF_Ar() override { return F_Ar_Here; }
};
