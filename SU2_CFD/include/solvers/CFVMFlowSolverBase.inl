...
template <class V, ENUM_REGIME R>
void CFVMFlowSolverBase<V, R>::BC_Sym_Plane_Residual(CGeometry* geometry, CSolver** solver_container,
  ...
  su2double Area, ProjVelocity_i, *V_reflected, *V_domain, Normal[MAXNDIM] = {0.0}, UnitNormal[MAXNDIM] = {0.0}, UnitNormalVertex[MAXNDIM] = {0.0};
  ...
  iVertex = geometry->nodes->GetVertex(iPoint,val_marker);
    ...
    if (true/*geometry->nodes->GetDomain(iPoint)*/) {
      ...
      ProjVelocity_i = nodes->GetProjVel(iPoint, UnitNormalVertex);
      su2double VB = 0;
      for (iDim = 0; iDim < nDim; ++iDim) {
        VB += (V_reflected[iDim+1] - ProjVelocity_i*UnitNormal[iDim]) * UnitNormal[iDim];
      }

      su2double Density_i = V_domain[prim_idx.Density()];
      su2double Enthalpy_i = V_domain[prim_idx.Enthalpy()];
      su2double Pressure_i = V_domain[prim_idx.Pressure()];
...
//      }
            ...
            residualBuffer[0] = Density_i * VB * Area;
            for (iDim = 0; iDim < nDim; ++iDim) {
              residualBuffer[iDim+1] = (Density_i*VB*V_domain[iDim+1] + Pressure_i*UnitNormal[iDim]) * Area;
            }
            residualBuffer[nDim+1] = Density_i * VB * Enthalpy_i * Area;

      if (implicit) {
        su2double V2 = nodes->GetVelocity2(iPoint);
        int index;
        /* mass */
        for (iDim = 0; iDim < nDim; ++iDim) {
          index = MAXNVAR + iDim + 1;
          residualBuffer[index] = UnitNormal[iDim] - UnitNormalVertex[iDim];
        }

        /* energy */

        index = MAXNVAR + (nDim+1)*nVar + 0;
        residualBuffer[index] = VB * (0.5*Gamma_Minus_One*V2 - Enthalpy_i);

        for (iDim = 0; iDim < nDim; ++iDim) {
          index = MAXNVAR + (nDim+1)*nVar + iDim + 1;
          residualBuffer[index] = Enthalpy_i*(UnitNormal[iDim] - UnitNormalVertex[iDim]) - VB*Gamma_Minus_One*V_domain[iDim+1];
        }

        index = MAXNVAR + (nDim+1)*nVar + nDim + 1;
        residualBuffer[index] = VB * Gamma;

        /* momentum */

        for (iDim = 0; iDim < nDim; ++iDim) {
          index = MAXNVAR + (iDim+1)*nVar + 0;
          residualBuffer[index] = 0.5*Gamma_Minus_One*V2*UnitNormal[iDim] - VB*V_domain[iDim+1];
        }

        for (iDim = 0; iDim < nDim; ++iDim) {
          index = MAXNVAR + (iDim+1)*nVar + nDim+1;
          residualBuffer[index] = Gamma_Minus_One*UnitNormal[iDim];
        }

        for (iDim = 0; iDim < nDim; ++iDim) {
          for (int jDim = 0; jDim < nDim; ++jDim) {
              index = MAXNVAR + (iDim+1)*nVar + jDim+1;
              residualBuffer[index] = V_domain[iDim+1]*(UnitNormal[jDim] - UnitNormalVertex[jDim]) - Gamma_Minus_One*UnitNormal[iDim]*V_domain[jDim+1];
          }
          index = MAXNVAR + (iDim+1)*nVar + iDim+1;
...
void CFVMFlowSolverBase<V, R>::BC_Custom_Weak(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
  ...
  if (VerificationSolution) {
    ...
    su2double Pressure, P_Exit, Velocity[MAXNDIM],
        Velocity2, Entropy, Density, Energy, Riemann, Vn, SoundSpeed, Mach_Exit, Vn_Exit,
        Area, UnitNormal[MAXNDIM], Normal[MAXNDIM];
    ...
    for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
      ...
      if (geometry->nodes->GetDomain(iPoint)) {
        ...
        su2double Solution[MAXNVAR] = {0.0}, sqvel = 0.0;
        VerificationSolution->GetBCState(coor, time, Solution);
        for (iDim = 0; iDim < nDim; ++iDim) {
          V_outlet[prim_idx.Velocity() + iDim] = Solution[iDim + 1] / Solution[0];
          sqvel += pow(V_outlet[prim_idx.Velocity() + iDim],2);
        }
        V_outlet[prim_idx.Pressure()] = Gamma_Minus_One * (Solution[nVar-1] - 0.5 * Solution[0] * sqvel);
        V_outlet[prim_idx.Temperature()] = V_outlet[prim_idx.Pressure()] / (Solution[0] * Gas_Constant);
        V_outlet[prim_idx.Density()] = Solution[0];
        V_outlet[prim_idx.Enthalpy()] = (Solution[nVar-1] + V_outlet[prim_idx.Pressure()]) / Solution[0];
...
template <class V, ENUM_REGIME R>
void CFVMFlowSolverBase<V, R>::BC_Custom_Weak_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                            CNumerics* visc_numerics, CConfig* config, unsigned short val_marker,
                                            unsigned long val_element, unsigned short iNode,
                                            su2double* residualBuffer) {
